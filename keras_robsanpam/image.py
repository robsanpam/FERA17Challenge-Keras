...
class ImageDataGeneratorMod(object):
...
class PickleImageDataGenerator(object):
    """Generate minibatches of image data with real-time data augmentation."""

    def __init__(self):
        pass

    def pickle_flow_from_directory(self, directory,
                            input_size=None,
                            classes=None, class_mode='categorical',
                            batch_size=32, shuffle=True, seed=None,
                            follow_links=False,
        ...
        if input_size is None:
            raise ValueError('The pickled image size must be entered explicitly '
                             'on input_size. E.g. (16, 112, 112, 3) for a pack '
                             'of 16 images of size 112x112 with 3 channels.')        
        return DirectoryIteratorPickle(
                directory, self,
...
class DirectoryIteratorPickle(Iterator):
    """Iterator capable of reading images from a directory on disk.
    # Arguments
        directory: Path to the directory to read images from.
            Each subdirectory in this directory will be
        ...
        image_data_generator: Instance of `ImageDataGenerator`
            to use for random transformations and normalization.
        input_size: tuple of integers, dimensions to resize input images to.
        classes: Optional list of strings, names of sudirectories
            containing images from each class (e.g. `["dogs", "cats"]`).
            It will be computed automatically if not set.
        class_mode: Mode for yielding the targets:
    ...
    def __init__(self, directory, image_data_generator,
        ...
        self.image_shape = tuple(self.input_size)
        self.classes = classes
        ...
        white_list_formats = {'png', 'jpg', 'jpeg', 'bmp', 'pickle'}
        ...
        for subdir in classes:
            ...
            for root, _, files in _recursive_list(subpath):
                for fname in files:
                    ...
                    if is_valid:
                        if(self.class_mode == 'multilabel'):
                            fileid = os.path.join(subdir, fname) # AU1 / FERA17_TR....
                            self.classes[i] = self.multilabel_classes[fileid]
                        else:
                            self.classes[i] = self.class_indices[subdir]
                        i += 1
                        ...
                        absolute_path = os.path.join(root, fname)
                        self.filenames.append(os.path.relpath(absolute_path, directory))
        super(DirectoryIteratorPickle, self).__init__(self.samples, batch_size, shuffle, seed)
    ...
    def next(self):
        ...
        for i, j in enumerate(index_array):
            fname = self.filenames[j]
            x = pickle.load(open(os.path.join(self.directory, fname), 'rb'))
            batch_x[i] = x
        # build batch of labels
        if self.class_mode == 'sparse':
            batch_y = self.classes[index_array]
