        ...
        for f in range(frame_count):
            if math.floor(f % steps) == 0 or take_all_frames:
                ...
                for k in range(num_real_image_channel):
                    resizedImage = cv2.resize(frame[:, :, k], (width, height), interpolation=cv2.INTER_AREA)
                ...
                if dense_optical_flow:
                    ...
                    image_with_flow = np.append(image_with_flow, frame_flow, axis=2)
...
def process_info(jobs, qty_labels, video_list,
                 ...
                 dense_optical_flow=False, n_channels=3,
                 pack=15, rescale=1./255):    
    
    if dense_optical_flow:
        # needs 3 additional channels for the optical flow with one exception:
        n_channels = 6
    ...
    for limits in jobs:
        ...
        for filename in files:
            ...
            # Add black frames to video
            black_frames = np.zeros((pack//2, v[0].shape[0], v[0].shape[1], v[0].shape[2]))
            v = np.append(black_frames, v, axis=0)
            v = np.append(v, black_frames, axis = 0)

            # Add blank to labels
            black_labels = np.zeros((pack//2, labels.shape[1]))
            labels = np.append(black_labels, labels, axis=0)
            labels = np.append(labels, black_labels, axis = 0)
            
            initial_frame = 0
            final_frame = initial_frame + pack
            
            while final_frame <= len(v):
                if not dense_optical_flow:
                    images = v[initial_frame:final_frame,:,:,:3]
                else:
                    images = v[initial_frame:final_frame,:,:,3:]
                ...
                label = labels[initial_frame+pack//2]
                
                # Pack whole path
                pack_path = result_dir+filename.split(".")[0]+"_%05d"%(initial_frame)+".pickle"
                # Pack path for dictionary
                dict_key = result_dir.split("/")[-2:-1][0]+"/"+filename.split(".")[0]+"_%05d"%(initial_frame)+".pickle"
                ...
                try:
                    ...
                    print('Unable to save data to',pack_path, ':', e)
                    raise
...
#                 break # To only process 1 image

#             break # To only process 1 video
...
#         break   # To only process 1 job 

def create_dataset(orig_db_path, main_dir,
                   target_h, target_w,
                   view, dense_optical_flow, pack, rescale=1./255):
    
    comm = MPI.COMM_WORLD
    rank = comm.Get_rank()

    # Name of directory for new dataset
    dataset_id = 'FERA17DB-3D-pack'+str(pack)+'-OF'+str(dense_optical_flow)[0]+'-h'+str(target_h)+'-w'+str(target_w)+'-v'+str(view)+'/'
...
def main():
    create_dataset(orig_db_path="/home/robsanpam/storage/Google Drive/Tesis/db/FERA17/", 
                   main_dir="/home/robsanpam/Desktop/FERA17_db/",
                   target_h=56, target_w=56,
                   view=6, dense_optical_flow=True, pack=15, rescale=1./255)
